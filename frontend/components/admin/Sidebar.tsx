...
export default function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  return (
    <>
        ...
        <aside 
            className={`fixed inset-y-0 left-0 z-50 flex w-[260px] flex-col bg-apomacy-dark shadow-xl transition-transform duration-300 ease-in-out md:translate-x-0 ${
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <Link
                  key={item.label}
                  href={item.href}
                  onClick={() => setIsOpen(false)} // Tutup sidebar otomatis setelah klik menu di HP
