...
import { 
    Search, Eye, X, Receipt, ShoppingBag, Clock, Truck, Store, Bell, CheckCircle2, ChevronLeft, ChevronRight, PackageCheck 
} from "lucide-react";
...
export default function KasirDashboardPage() {
    ...
    useEffect(() => {
        const names   = ["Bpk. Budi Santoso", "Siti Aminah", "Andi Saputra", "Ibu Ratna", "Reza Rahadian", "Diana Putri", "Hendra Gunawan", "Lestari Wulandari"];
        const meds    = [
            { name: "Paracetamol 500mg",  price: 5000  },
            { name: "Vitamin C 1000mg",   price: 15000 },
            { name: "Amoxicillin 500mg",  price: 2000  },
            { name: "Ibuprofen 400mg",    price: 8000  },
            { name: "Antasida Doen",      price: 6500  },
            { name: "OBH Combi Batuk",    price: 12000 },
        ];
        const methods = ["Cash", "QRIS", "Transfer Bank", "Kartu Debit"];

        const generatedData: TransaksiDashboard[] = Array.from({ length: 60 }).map((_, i) => {
            const type = i % 4 === 0 ? "Offline" : "Online";
            ...
            if (type === "Offline") {
                ...
                const r = i % 7;
                if (r === 0)      status = "Obat sedang Disiapkan"; 
                else if (r === 1) status = "Pesanan Baru";
                else if (r === 2) status = "Dalam perjalanan";
                else if (r === 3) status = "Obat menunggu di ambil";
                else              status = "Selesai";
            }
            ...
            return {
                ...
                items: [{ name: med.name, qty, price: med.price }],
                subtotal: med.price * qty,
                paymentMethod: methods[i % methods.length],
                status,
                date: `2026-05-19 ${String(hour).padStart(2, "0")}:${String(min).padStart(2, "0")}`,
            };
        });

        setTransactions(generatedData);
    ...
    const renderAction = (trx: TransaksiDashboard) => {
        ...
        return (
            <span style={getStatusStyle(trx.status)} className="inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold">
                {trx.status}
            </span>
        );
    };
        ...
        <div className="flex flex-col gap-6 w-full h-[calc(100vh-100px)] relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 shrink-0">
                {[
                    ...
                    { icon: <Clock size={24} />,   bg: "#fffbeb", color: "#d97706", label: "Sedang Disiapkan",     value: `${transactions.filter(t => t.status === "Obat sedang Disiapkan").length} Pesanan` },
            ...
            <div className="flex-1 bg-white rounded-2xl border border-outline-variant p-6 shadow-sm flex flex-col min-h-0">
                ...
                <div className="w-full border border-outline-variant rounded-xl overflow-hidden bg-white shadow-xs flex-1 min-h-0">
                    <div className="overflow-y-auto overflow-x-auto max-h-[580px] block w-full scrollbar-thin scrollbar-thumb-gray-300">
                        <table className="w-full text-left border-collapse min-w-[800px] table-fixed">
                            <thead className="sticky top-0 z-20 bg-gray-50 border-b border-outline-variant shadow-xs">
                                <tr className="text-[11px] font-bold uppercase tracking-wider text-on-surface-variant">
                                    ...
                                    <th className="px-4 py-3 bg-gray-50 w-[240px] whitespace-nowrap">
                                        {activeTab === "Pesanan Baru" || activeTab === "Diproses" ? "Aksi" : "Status Pesanan"}
                                    </th>
                                </tr>
                            </thead>
                                    ...
                                    displayedData.map(trx => (
                                        <tr
                                            ...
                                            className={`hover:bg-surface-container-low transition-colors ${
                                                trx.status === "Obat sedang Disiapkan" ? "bg-amber-50/30" : ""
                                            }`}
                                        >
            ...
            {mounted && selectedDetail && (
                <PortalModal onClose={() => setSelectedDetail(null)}>
                    <div style={{ background: "#fff", borderRadius: 20, width: 400, maxWidth: "100%", boxShadow: "0 24px 80px rgba(0,0,0,0.25)", overflow: "hidden" }}>
                        <div style={{ background: "var(--color-apomacy-primary, #0d6efd)", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px" }}>
                                ...
                                <thead>
                                    <tr style={{ borderBottom: "1px solid #e5e7eb" }}>
                                        {["Item", "Qty", "Total"].map((h, i) => (
                                            <th key={h} style={{ padding: "10px 0 8px", fontSize: 10, fontWeight: 600, color: "#9ca3af", textTransform: "uppercase", textAlign: i === 0 ? "left" : i === 1 ? "center" : "right" }}>{h}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
